...
export default function Login() {
    ...
    const handleDemo = async () => {
        ...
        setPassword("avenzaDemo@123");
        try {
            setLoading(true);

            const res = await emailLogin({
                email: "avenzademo@avenza.com",
                password: "avenzaDemo@123",
            });
            ...
            <div className={`${isDark ? "bg-[#0F172A90] shadow-lg shadow-[#0F172A] border-gray-800 border-2" : "bg-[#FFFFFF60]"} flex flex-row items-center rounded-4xl h-fit lg:gap-30 gap-5 relative`}>
                <div className={`md:min-h-[75dvh] min-h-[70dvh] md:max-w-md max-w-lg rounded-4xl md:rounded-r-none p-5 sm:p-8 flex flex-col items-center`}>
                    ...
                    <div className="mt-8 space-y-4 w-full">
                        <div>
                            <div className={`${isDark ? "bg-[#0F172A] border-gray-800 shadow-[#0F172A] border-2" : "bg-[#F9FAFB] border border-[#E5E7EB] shadow-gray-200"} flex flex-row shadow-sm rounded-xl pl-3 items-center gap-2 overflow-hidden  ${errors.email && "border border-red-600"}`}>
                                ...
                                <input
                                    ...
                                    className={`${isDark ? "text-gray-100" : "text-[#374151]"} w-full h-full py-3 pr-4 text-[#374151] font-semibold focus:outline-none placeholder:font-semibold placeholder:text-[#9CA3AF]`}
                                />
                            </div>
                            {errors.email && (
                        ...
                        <div className="w-full flex items-center justify-center">
                            <button
                                className={`${isDark ? "text-rose-400" : "text-rose-500"} font-semibold hover:underline flex items-center justify-center gap-2`}
                                disabled={loading}
                                onClick={handleDemo}
                            >
                ...
                <div className={`hidden md:block min-w-xs relative self-stretch rounded-4xl ${isDark ? "bg-[#a7366b]" : "bg-[#e47eae]"} `}>
